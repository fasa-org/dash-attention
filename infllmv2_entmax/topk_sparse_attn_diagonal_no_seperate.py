...
def forward_kernel(
    ...
    for j in range(real_q_loop):
        ...
        for i in range(real_topk):
            ...
            e = tl.load(e_ptr_base + c_idx * stride_em, mask=(c_idx < tl.cdiv(pid_q_j + 1, BLOCK_SIZE_K)) & (c_idx < e_len) & (off_h < NUM_SHARE_Q_HEADS), other=0) # [BLOCK_SIZE_H]
...
def backward_dkdv(
    ...
    for i in range(0, act_q_len, BLOCK_SIZE_Q):
        ...
        e = tl.load(
            ...
            mask=(off_q < act_q_len - i)[:, None] & (pid_k < tl.cdiv(idx_q + 1, BLOCK_SIZE_K))[:, None] & (pid_k < e_len),
            other=0,
        )
        # compute qk
        ...
        de = tl.sum(ds, axis=1) 
        de_save_mask = (pid_k < tl.cdiv(k_len + 1, BLOCK_SIZE_K)) & (tl.sum(tl.where(idx_q[:, None] >= off_k[None, :], 1, 0), axis=1) > 0)
        pos_mask = (pid_k < tl.cdiv(idx_q + 1, BLOCK_SIZE_K))
...
def backward_dq(
    ...
    for j in range(real_q_loop):
        ...
        for i in range(real_topk):
            ...
            e = tl.load(e_ptr_base + c_idx * stride_em, mask=(c_idx < tl.cdiv(pid_q_j + 1, BLOCK_SIZE_K)) & (c_idx < e_len) & (off_h < NUM_SHARE_Q_HEADS), other=0)
            # compute qk
            qk = tl.zeros((BLOCK_SIZE_H, BLOCK_SIZE_K), dtype=tl.float32)
            qk += e[:, None] * qk_scale
            qk += tl.where((pid_q_j >= c + off_k)[None, :], 0, float("-inf"))
