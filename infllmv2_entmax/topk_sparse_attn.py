...
def backward_dkdv(
    ...
    for i in range(0, act_q_len, BLOCK_SIZE_Q):
        ...
        de = tl.sum(ds, axis=1) 
        de_save_mask = (pid_k < (k_len // BLOCK_SIZE_K)) & (tl.sum(tl.where(idx_q[:, None] >= off_k[None, :], 1, 0), axis=1) > 0)
        pos_mask = (pid_k < (idx_q // BLOCK_SIZE_K) - 1)
        tl.store(de_to_save_ptrs, de.to(tl.bfloat16), mask=de_save_mask & pos_mask)
        # cast dtype
        p = p.to(do.dtype)
        ds = ds.to(q.dtype)
        # update dk and dv
...
def test():
    ...
    # this cannot match because there is grad for attn sink and sliding window in dd_ref, but not in dd_tr
