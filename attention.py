...
if HAVE_TE:
    from megatron.core.transformer.custom_layers.transformer_engine import SplitAlongDim, TEDotProductAttention
else:
    SplitAlongDim = None
...
class SelfAttentionSubmodules:
    ...
    ### new params
    indexer_q: Union[ModuleSpec, type] = None
...
class Attention(MegatronModule, ABC):
    ...
    def _checkpointed_attention_forward(
        ...
        def custom_forward(*inputs):
            ...
            # gated = inputs[7] if len(inputs) > 7 else None
    ...
    def forward(
        ...
        # self or cross attn.
        # query, key, value, gated = self.get_query_key_value_tensors(hidden_states, key_value_states)
...
class SelfAttention(Attention):
    ...
    def __init__(
        ...
        # NOTE(Yuxiang Huang): use a learned q for local attn
        self.indexer_q = torch.nn.Parameter(
            torch.zeros(self.kv_projection_size, dtype=self.config.params_dtype)
        )
    ...
    def get_query_key_value_tensors(self, hidden_states, key_value_states=None):
        ...
        # gated, _  = self.linear_gated(hidden_states)
        ...
        # gated = gated.reshape(query.size(0), query.size(1), -1,2) #!如果要改专家数的话, 这里记得改
    ...
    def forward(
        ...
        indexer_q = self.indexer_q.view(1, 1, -1).expand(hidden_states.shape[0], hidden_states.shape[1], -1) # 沒有lin proj
        indexer_q = indexer_q.reshape(indexer_q.shape[0], -1, self.config.kv_channels)
        ...
        if rotary_pos_emb is not None:
            ...
            indexer_q = apply_rotary_pos_emb(
                indexer_q,
                q_pos_emb,
                config=self.config,
                cu_seqlens=cu_seqlens_q,
