...
def load_qa(dataset, path, demo_path, max_test_samples=None, popularity_threshold=None, shots=0, seed=42):
    """
    ...
    if max_test_samples is not None:
        ...
        keys = set(data[key])
        keys = rng.sample(sorted(keys), min(max_test_samples, len(keys)))
...
def load_multi_lexsum(dataset, path=None, shots=0, max_samples=None, seed=42):
    ...
    all_data = load_from_disk("/home/test/test01/hyx/multi_lexsum")
...
def load_data(args, dataset, path=None, demo_path=None):
    ...
    if "popqa" in dataset:
        ...
        data = load_qa(dataset, path, demo_path, max_test_samples=args.max_test_samples, popularity_threshold=popularity_threshold, shots=args.shots, seed=args.seed)
    elif any([x in dataset for x in ["nq", "hotpotqa", "triviaqa"]]):
        data = load_qa(dataset, path, demo_path, max_test_samples=args.max_test_samples, shots=args.shots, seed=args.seed)
