...
from .topk_sparse_attention_decode import topk_sparse_attention_decode
...
# handler = logging.FileHandler(f"/home/test/test01/hyx/olmes/activated_{run_id}.log")
# NOTE(Yuxiang Huang): This logs the activated density 
handler = logging.FileHandler(f"/yourpath/activated_{run_id}.log")
# handler = logging.FileHandler(f"/home/test/test01/hyx/olmes/activated_{run_id}.log")
# handler = logging.FileHandler(f"/home/test/test01/hyx/Reasoning/activated_{run_id}.log")
...
class CompressK(torch.nn.Module):
    ...
    def forward(self, indexer_q: torch.Tensor, k: torch.Tensor, cu_seqlens):
        ...
        # use mean query as the input
        filtered_q = filtered_q.mean(dim=1)

        # local attention
        comp_score = torch.einsum('nhd,nmhd->nmh', filtered_q, filtered_k).float()
        comp_prob = torch.nn.functional.softmax(comp_score * (filtered_k.shape[-1] ** -0.5), dim=1)
        compress_k = torch.einsum('nmh,nmhd->nhd', comp_prob, filtered_k.float()).to(torch.bfloat16)
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        # self.indexer_q = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.attention_bias)
        self.indexer_q = torch.nn.Parameter(
            torch.zeros(self.num_key_value_heads * self.head_dim, dtype=torch.bfloat16)
        )

        self.kernel_size = 16
        self.kernel_stride = 16
        # self.kernel_size = 32
        # self.kernel_stride = 32
        self.init_blocks = 1
        self.block_size = 16
        # self.block_size = 32
        ...
        #     self.num_key_value_heads * self.head_dim, self.num_key_value_heads, bias=config.attention_bias
        # )
...
try:
    ...
    from .stage1 import compressed_attention_fwd
    from .transform_score import transform_score
...
def compressed_attention(
    ...
    with torch.no_grad():
        ...
        # NOTE(Yuxiang Huang): tune this to change density. Higher scaling_factor -> sparser
        ...
        # else:
        #     scaling_factor = 0.26
        if q.shape[0] == 1: # do not support batching
            _k = k.repeat_interleave(q.shape[1] // k.shape[1], dim=1)
            score = torch.matmul((q.shape[-1] ** (-0.5)) * q.transpose(0, 1), _k.transpose(0, 1).transpose(1, 2)) * scaling_factor * 1.44269504
            # score = torch.matmul((q.shape[-1] ** (-0.5)) * (q / q.norm(dim=-1, keepdim=True)).transpose(0, 1), (_k / _k.norm(dim=-1, keepdim=True)).transpose(0, 1).transpose(1, 2)) * scaling_factor * 1.44269504
        else: # prefill
            # compute attention score
            # score = compressed_attention_fwd(
            #     q / q.norm(dim=-1, keepdim=True), k / k.norm(dim=-1, keepdim=True), kernel_size, kernel_stride, 
            #     cu_seqlens_q, cu_seqlens_k,
            #     max_seqlen_q, max_seqlen_k,
            #     scaling_factor * q.shape[-1] ** (-0.5)
            # ) # *1.44269504 inside the kernel
            score = compressed_attention_fwd(
        ...
        score = triton_entmax(score, alpha=alpha, n_iter=3)
        ...
        pooled_score = score_
        pooled_score = F.pad(pooled_score, (0, 1), value=0)

        # GQA + NoSTE
        pooled_score = pooled_score.reshape(k.shape[1], -1, pooled_score.shape[-2], pooled_score.shape[-1])
        group_size = pooled_score.shape[1]
        pooled_score = pooled_score.mean(dim=1)
        pooled_score = pooled_score.repeat_interleave(group_size, dim=0)
        if is_prefilling:
            ...
            block_score = F.pad(score, (0, 1), value=0).clone()
            if (cache_lens.item() + 1) % block_size == 0:
                block_score[:, :, -2] = float("inf")
            else:
                block_score[:, :, -2:] = float("inf")
            # block_score[:, :, 0] = float("inf")

        if is_prefilling:
            logger.info(f"average activated: {(block_score>0).sum(dim=-1)[:, 4096:].float().mean().item() * 16}, num_tokens: {block_score.shape[1] - 4096}")
        else:
            logger.info(f"average activated: {(block_score>0).sum(dim=-1)[:, :].float().mean().item() * 16}, num_tokens: 1")
...
class LlamaSdpaAttention(LlamaAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        key_states = self.k_proj(hidden_states)
        value_states = self.v_proj(hidden_states)
        indexer_q_states = self.indexer_q.unsqueeze(0).unsqueeze(1).expand(hidden_states.shape[0], hidden_states.shape[1], -1)

        # indexer_q_states[...] = 0
        # indexer_q_states = self.indexer_q(hidden_states_pre_norm)

        # dt_states = self.dt_proj(value_states)
        # dt_states = torch.exp(self.A * F.softplus(dt_states))
        # cis = dt_states.to(hidden_states.dtype) 
        cis = torch.zeros((bsz, q_len, self.num_key_value_heads), dtype=key_states.dtype, device=key_states.device)


        query_states = query_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        key_states = key_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        value_states = value_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        indexer_q_states = indexer_q_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        ...
        query_states_no_rope = query_states.unsqueeze(2).repeat(1, 1, 2, 1, 1).flatten(1, 2)
        key_states_no_rope = key_states.clone()
        ...
        if not self.use_nope:
            indexer_q_states = apply_rotary_pos_emb_single(indexer_q_states, cos, sin)
        ...
        if past_key_value is not None:
            ...
            if q_len > 1: # prefill
                self.q_cache = []
                self.q_cache.append(indexer_q_states)
            else: # decode
                self.q_cache.append(indexer_q_states)
            indexer_q_states = torch.cat(self.q_cache, dim=2)
        ...
        if query_states.device.type == "cuda":
            ...
            indexer_q_states = indexer_q_states.contiguous()

        # query_states = query_states.unsqueeze(2).repeat(1, 1, 2, 1, 1).flatten(1, 2)
        attn_output = self._sparse_attention_forward(
                query_states.transpose(1, 2), key_states.transpose(1, 2), value_states.transpose(1, 2), indexer_q_states.transpose(1, 2), cis, attention_mask, q_len, dropout=0.0,
                no_rope_param=no_rope_param,  # if past_key_value is not None else None,
                past_key_value=None)
        # aos = attn_output.shape

        # attn_output = attn_output.view(aos[0], aos[1], -1, 2, aos[3])[:, :, :, 0, :]
    ...
    def _sparse_attention_forward(
        ...
        if attention_mask is not None:
            ...
            # TODO: only support single batch
            indexer_q_states = indexer_q_states.squeeze(0)

            cu_seqlens_q, cu_seqlens_k = cu_seq_lens
            max_seqlen_in_batch_q, max_seqlen_in_batch_k = max_seq_lens
            if no_rope_param != None:
                # TODO: only support single batch prefill
                no_rope_param['query_states_no_rope'] = no_rope_param['query_states_no_rope'].squeeze(0).transpose(0, 1)
                no_rope_param['key_states_no_rope'] = no_rope_param['key_states_no_rope'].squeeze(0).transpose(0, 1)
            if past_key_value==None:
                # compress_k use varlen form
                if self.use_nope:
                    compressed_k, compressed_cu_seqlens = self.compress_k(indexer_q_states, no_rope_param['key_states_no_rope'] ,cu_seqlens_k)
                else:
                    compressed_k, compressed_cu_seqlens = self.compress_k(indexer_q_states, key_states ,cu_seqlens_k)
    ...
    def get_compress_k(self, indexer_q_states, key_states, attention_mask, past_key_value):
        """
        ...
        if is_prefilling:
            ...
            unpadded_indexer_q_states, _, _, _ = _unpad_one_tensor(indexer_q_states,attention_mask=attention_mask)
            # Compress the keys
            compressed_k, compressed_cu_seqlens = self.compress_k(unpadded_indexer_q_states, unpadded_key_states, cu_seqlens)
            ...
            for i in range(len(compressed_cu_seqlens)-1):
                ...
                no_compress_q_list.append(unpadded_indexer_q_states[cu_seqlens[i]+no_compress_k_start:cu_seqlens[i+1]].clone())
    ...
    def sparse_forward(self,
        ...
        topk_idx, max_k, pooled_score = compressed_attention(
            query_layer if not self.use_nope else no_rope_param['query_states_no_rope'],
            compressed_k,
            compressed_k.clone(),
            self.kernel_size,
            self.kernel_stride,
        ...
        pooled_score = torch.log(pooled_score / self.block_size) # 允许出现-inf
        mask = torch.isfinite(pooled_score)
        mask_cnt = mask.sum(dim=-1, keepdim=True)
        ps_valid = torch.where(mask, pooled_score, torch.zeros_like(pooled_score))
        ps_mean = ps_valid.sum(dim=-1, keepdim=True) / mask_cnt.clamp(min=1)

        sigma = 1e8
        pooled_score = (pooled_score - ps_mean.detach()) / sigma

        # pooled_score[mask] = 0

        # print(f"average activated: {(topk_idx>=0).sum(dim=-1)[:, 4096:].float().mean().item() * 64}")
        if query_layer.shape[0] > 1:
            topk_attn_output = topk_sparse_attention(query_layer, key_layer, value_layer, topk_idx.to(torch.int32), pooled_score, block_size=self.block_size, cu_seqlens=cu_seqlens_q)
        else:
            topk_attn_output = topk_sparse_attention_decode(query_layer, key_layer[None, ...], value_layer[None, ...], topk_idx.to(torch.int32), pooled_score, self.block_size, cu_seqlens_k[1:] - cu_seqlens_k[:-1], query_layer.shape[-1] ** (-0.5))
