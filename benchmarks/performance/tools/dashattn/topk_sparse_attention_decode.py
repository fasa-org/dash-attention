...
def forward_kernel(
    ...
    e_ptr_base = e_ptr + (pid_kh * NUM_SHARE_Q_HEADS + off_h) * stride_eh + pid_b * stride_eb
    # init statistics
    ...
    for i in range(real_topk):
        ...
        c_idx = tl.load(t_ptr).to(tl.int32) 
        ...
        e = tl.load(e_ptr_base + c_idx * stride_em, mask=(c_idx < e_len - 2) & (off_h < NUM_SHARE_Q_HEADS), other=0)
        # compute qk
...
def topk_sparse_attention_decode(
    ...
    Args:
        ...
        pooled_score (torch.Tensor): shape [num_q_heads, batch_size, *]
    ...
    forward_kernel[grid](
        ...
        v.stride(2),
        v.stride(3),
        pooled_score.stride(0),
        pooled_score.stride(1),
        pooled_score.stride(2),
        pooled_score.shape[-1],
        topk_idx.stride(0),
        topk_idx.stride(1),
        topk_idx.stride(2),
        o.stride(0),
