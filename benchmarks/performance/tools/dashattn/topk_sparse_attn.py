...
def forward_kernel(
    ...
    for j in range(real_q_loop):
        ...
        # set base ptr for e
        e_ptr_base = e_ptr + (pid_h + tl.arange(0, BLOCK_SIZE_H)) * stride_eh + (q_start + pid_q_j) * stride_en
        # init statistics
        ...
        for i in range(real_topk):
            ...
            c_idx = tl.load(t_ptr_j).to(tl.int32)
            c = c_idx * BLOCK_SIZE_K
            t_ptr_j = t_ptr_j + stride_tk
            # load k
            k = tl.load(
                tl.advance(k_ptrs, (0, c)), boundary_check=(1, 0), padding_option="zero"
            )
            # load e, e=0 for sliding window
            e = tl.load(e_ptr_base + c_idx * stride_em, mask=(c_idx < pid_q_j // BLOCK_SIZE_K - 1) & (c_idx < e_len) & (off_h < NUM_SHARE_Q_HEADS), other=0) # [BLOCK_SIZE_H]
            # q: (BLOCK_SIZE_H, BLOCK_SIZE_D), k: (BLOCK_SIZE_D, BLOCK_SIZE_K)
            # compute qk
            ...
            m_ij_safe = tl.where(all_masked, 0.0, m_ij)
            ...
            acc_o_scale = tl.where(all_masked, 1.0, acc_o_scale)   
            acc_o = acc_o * acc_o_scale[:, None]
            ...
            p = p.to(v.dtype)
            acc_o += tl.dot(p, v)
            # if (pid_h == 0) & (pid_q == 16381) & (i == 1):
            #     tl.device_print("p: ", qk)
            # update statistics
            # m_i = m_ij
            # lse_i = m_ij + tl.math.log2(tl.exp2(lse_i - m_ij) + l_ij)
            lse_new = m_ij_safe + tl.math.log2(tl.exp2(lse_i - m_ij_safe) + l_ij)
            m_i  = tl.where(all_masked, m_i,  m_ij)
            lse_i = tl.where(all_masked, lse_i, lse_new)
        # final scale
        has_attn = real_topk > 0
        scale = tl.where(has_attn, tl.exp2(m_i - lse_i), 0.0)
        ...
        lse_out = tl.where(has_attn, lse_i, float("-inf"))
        tl.store(lse_ptrs, lse_out, mask=off_h < NUM_SHARE_Q_HEADS)
...
def backward_dkdv(
    ...
    # assume BLOCK_SIZE_K is block_size
    e_ptrs = e_ptr + pid_h * stride_eh + pid_k * stride_em + q_start * stride_en
    de_ptrs = de_ptr + pid_sh * stride_des + pid_kh * stride_deh + pid_k * stride_dem
    # offsets
    ...
    for i in range(0, act_q_len, BLOCK_SIZE_Q):
        ...
        e = tl.load(
            e_ptrs + idx_q[:, None] * stride_en,
            mask=(off_q < act_q_len - i)[:, None] & (pid_k < idx_q // BLOCK_SIZE_K - 1)[:, None] & (pid_k < e_len),
            other=0,
        )
        # compute qk
        ...
        ds = sm_scale * p * (dp - d)
        # compute de
        de_to_save_ptrs = de_ptrs + idx_q * stride_den # [BLOCK_SIZE_Q]
        de = tl.sum(ds, axis=1) #/ tl.sum(idx_q[:, None] >= off_k[None, :], axis=1)
        de_save_mask = (pid_k < (k_len // BLOCK_SIZE_K)) & (tl.sum(idx_q[:, None] >= off_k[None, :], axis=1) > 0)
        tl.store(de_to_save_ptrs, de.to(tl.bfloat16), mask=de_save_mask)
        # cast dtype
        p = p.to(do.dtype)
        ds = ds.to(q.dtype)
        # update dk and dv
...
def backward_dq(
    ...
    for j in range(real_q_loop):
        ...
        # set base ptr for e
        off_h = tl.arange(0, BLOCK_SIZE_H)
        e_ptr_base = e_ptr + (pid_h + off_h) * stride_eh + (q_start + pid_q_j) * stride_en
        # sparse
        for i in range(real_topk):
            # get current block start index
            c_idx = tl.load(t_ptr_j).to(tl.int32)
            ...
            e = tl.load(e_ptr_base + c_idx * stride_em, mask=(c_idx < pid_q_j // BLOCK_SIZE_K - 1) & (c_idx < e_len) & (off_h < NUM_SHARE_Q_HEADS), other=0)
            # compute qk
            qk = tl.zeros((BLOCK_SIZE_H, BLOCK_SIZE_K), dtype=tl.float32)
            qk += e[:, None] * qk_scale
            qk += tl.where((pid_q_j >= c + off_k)[None, :], 0, float("-inf"))
...
def _topk_sparse_attention_fwd(
    ...
    e: torch.Tensor,  # [num_q_heads, total_len, *]
...
def _topk_sparse_attention_bwd(
    ...
    de = torch.zeros(
        num_share_q_heads, num_k_heads, score.shape[1], score.shape[2], device=k.device, dtype=k.dtype
    )
    batch_size = cu_seqlens_q.shape[0] - 1
    BLOCK_SIZE_K = triton.next_power_of_2(block_size)
    BLOCK_SIZE_Q = 64
    ...
    dv = dv.sum(0)
    # de = de.sum(0)
    de = de.transpose(0, 1).flatten(0, 1)
    # compute dq
...
class TopkSparseAttention(torch.autograd.Function):
    ...
    def forward(
        ...
        v: torch.Tensor,  # [total_len, num_k_heads, head_dim]
        topk_idx: torch.Tensor,  # [num_kv_heads, total_len, topk]
        score: torch.Tensor, # [num_kv_heads, total_len, *]
        block_size: int,
        cu_seqlens_q: torch.Tensor,
        cu_seqlens_k: torch.Tensor,
        max_seqlen_q: int,
        ...
        # breakpoint()
        ctx.save_for_backward(q, k, v, o, lse, cu_seqlens_q, cu_seqlens_k, topk_idx, score)
    ...
    def backward(ctx, do: torch.Tensor, *args) -> Any:
        q, k, v, o, lse, cu_seqlens_q, cu_seqlens_k, topk_idx, score = ctx.saved_tensors
        # import torch.distributed as dist
        # if dist.get_rank() == 0:
        #     breakpoint()
        # dist.barrier()
        ...
        assert block_size in {32, 64, 128, 256}
        dq, dk, dv, de = _topk_sparse_attention_bwd(
            o,
            do,
            lse,
            q,
        ...
        # if (de.isnan().any() or de.isinf().any()) and torch.distributed.get_rank() == 14:
        #     breakpoint()
        # torch.distributed.barrier()
        # if (dq.isnan().any() or dq.isinf().any()) and torch.distributed.get_rank() == 14:
        #     breakpoint()
        # torch.distributed.barrier()
        # if (dk.isnan().any() or dk.isinf().any()) and torch.distributed.get_rank() == 14:
        #     breakpoint()
        # torch.distributed.barrier()
        # if (dv.isnan().any() or dv.isinf().any()) and torch.distributed.get_rank() == 14:
        #     breakpoint()
        # torch.distributed.barrier()
        # breakpoint()
        return dq, dk, dv, None, de, None, None, None, None, None, None, None
...
def _forward_reference_case(block_size: int, N: int, requires_grad: bool):
    H = 2
    HQ = 16
    D = 128
    M = 256
    ...
    q = torch.randn((N, HQ, D), dtype=torch.bfloat16, device="cuda", requires_grad=True)
    k = torch.randn((N, H, D), dtype=torch.bfloat16, device="cuda", requires_grad=True)
    v = torch.randn((N, H, D), dtype=torch.bfloat16, device="cuda", requires_grad=True)
    d = torch.randn((HQ, N, M), dtype=torch.bfloat16, device="cuda", requires_grad=True)
    ...
    q_idx = torch.cat([
        (torch.arange(cu_seqlens[i + 1] - cu_seqlens[i], device=q.device) + 
            max_seqlen - (cu_seqlens[i + 1] - cu_seqlens[i])) // block_size
        for i in range(batch_size)
    ], dim=0)

    idxs = [0, 1, 10, 23, 47, 127, 254, 255]
    with torch.no_grad():
        d[:, :, 0] = 0
        d[:, :, -2:] = 0
    
    topk_idx = torch.tensor(idxs, dtype=torch.int, device='cuda').unsqueeze(0).unsqueeze(0).repeat(H, N, 1)
    topk_idx[topk_idx > q_idx[None, :, None]] = -1

    tr_out = topk_sparse_attention(q, k, v, topk_idx, d, block_size, cu_seqlens)[-1]

    all_k_pos = []
    all_m_pos = []
    for idx in idxs:
        _k = k[idx * block_size : (idx + 1) * block_size, :, :]
        in_block_len = _k.shape[0]
        k_pos = idx * block_size + torch.arange(0, in_block_len, device="cuda")
        all_k_pos.append(k_pos)
        all_m_pos += [idx] * in_block_len

    all_k_pos = torch.cat(all_k_pos)
    all_m_pos = torch.tensor(all_m_pos, device=all_k_pos.device, dtype=all_k_pos.dtype)

    q_pos = torch.arange(0, N, dtype=all_k_pos.dtype, device=all_k_pos.device)
    attn_mask = q_pos[:, None] >= all_k_pos[None, :]

    _k = k.transpose(0, 1).repeat_interleave(HQ // H, dim=0)
    _v = v.transpose(0, 1).repeat_interleave(HQ // H, dim=0)
    _kv_idx = all_k_pos.unsqueeze(0).unsqueeze(-1)
    _k = _k[:, all_k_pos, :]
    _v = _v[:, all_k_pos, :]
    _mask = torch.zeros_like(attn_mask, dtype=q.dtype)
    _mask[~attn_mask] = float("-inf")
    _s = (torch.matmul(q.transpose(0, 1).float(), _k.transpose(-2, -1).float()) + d[:, :, all_m_pos].float() + _mask[None, :, :].float()) / math.sqrt(D)
    torch_out = torch.matmul(torch.softmax(_s, dim=-1), _v.float()).to(torch.bfloat16)

    torch_out = torch_out.transpose(0, 1)[-1]
    torch.testing.assert_close(tr_out, torch_out, atol=5e-2, rtol=0.0)
    return q, k, v, d, tr_out, torch_out


def test():
    _forward_reference_case(block_size=16, N=4098, requires_grad=False)
    print("forward block_size=16 good")

    q, k, v, d, tr_out, torch_out = _forward_reference_case(
        block_size=64, N=16382, requires_grad=True
    )
    print("forward good")
    ...
    torch.testing.assert_close(dq_tr, dq_ref, atol=5e-2, rtol=0.0)
    torch.testing.assert_close(dk_tr, dk_ref, atol=5e-2, rtol=0.0)
    torch.testing.assert_close(dv_tr, dv_ref, atol=5e-2, rtol=0.0)
    # breakpoint()
    # torch.testing.assert_close(dd_tr, dd_ref, atol=5e-2, rtol=0.0)
