...
def compressed_attention(
    ...
    with torch.no_grad():
        ...
        if q.shape[0] == 1:
            q_heads = q.shape[1]
            k_heads = k.shape[1]
            comp_k = k.repeat_interleave(q_heads // k_heads, dim=1)
            comp_v = v.repeat_interleave(q_heads // k_heads, dim=1)
            score = torch.matmul((q.shape[-1] ** (-0.5)) * q.transpose(0, 1), comp_k.transpose(0, 1).transpose(1, 2))
            score = F.softmax(score, dim=-1)
            comp_attn_output = torch.einsum("hnm,mhd->nhd", score, comp_v)
        else:
            q_heads = q.shape[1]
            k_heads = k.shape[1]
            comp_v = v.repeat_interleave(q_heads // k_heads, dim=1)
            ...
            try:
                ...
                print(score.shape, comp_v.shape)
            
    return topk_idx, comp_attn_output
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        self.compress_kv = CompressKV(self.num_key_value_heads, self.head_dim, kernel_size=self.kernel_size, kernel_stride=self.kernel_stride)
        self.gate_proj = torch.nn.Linear(self.head_dim, 2,)
...
class LlamaSdpaAttention(LlamaAttention):
    ...
    def _sparse_attention_forward(
        ...
        if attention_mask is not None:
            ...
            if past_key_value==None:
                ...
                compressed_k, compressed_v, compressed_cu_seqlens = (self.compress_kv(kv,cu_seqlens_k) if max_seqlen_in_batch_k >= self.kernel_size else (None, None, None))
    ...
    def sparse_forward(self,
        ...
        if compressed_k is None:
            ...
            gated_score = self.gate_act(self.gate_proj(query_layer))
            attn_output = gated_score[..., 1:2] * topk_attn_output
            ...
            topk_idx, comp_attn_output = compressed_attention(
            ...
            gated_score = self.gate_act(self.gate_proj(query_layer))
            attn_output = gated_score[..., 0:1] * comp_attn_output + gated_score[..., 1:2] * topk_attn_output
...
LLAMA_ATTENTION_CLASSES = {
    ...
    "flash_attention_2": LlamaSdpaAttention,
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            inputs_embeds = self.embed_tokens(input_ids) * 12
...
class SparseLlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if self.config.pretraining_tp > 1:
            ...
            logits = self.lm_head(hidden_states[:, -num_logits_to_keep:, :] / (self.config.hidden_size / 256)).float()
