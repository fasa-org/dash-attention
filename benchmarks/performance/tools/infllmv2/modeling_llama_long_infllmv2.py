...
def calc_chunks_with_stride(cu_seqlen, chunk_size, kernel_stride):
    ...
    max_num_chunks_per_seq = torch.clamp(max_num_chunks_per_seq, min=0)
...
def compressed_attention(
    ...
    with torch.no_grad():
        ...
        # c_score = cis.permute(2, 0, 1) # (H, B, N)
        # q_len = c_score.shape[-1]
        # c_score = F.avg_pool1d(c_score, kernel_size=32, stride=16) # (H, B, num_blocks)
        # c_score = c_score.unsqueeze(2).repeat(1, 1, q_len, 1)
        # c_score = c_score.flatten(1, 2)
...
class LlamaSdpaAttention(LlamaAttention):
    ...
    def sparse_forward(self,
        ...
        nvtx.range_push("compressed_attention_block")
        ...
        # for h in range(2):
        #     diff = 0
        #     for i in range(topk_idx.shape[1] - 1):
        #         cur = set(topk_idx[h, i].tolist())
        #         nxt = set(topk_idx[h, i + 1].tolist())
        #         diff += len(cur ^ nxt)
        #     diff /= (topk_idx.shape[1] - 1)
        #     print(f"head {h}, diff: {diff}")
